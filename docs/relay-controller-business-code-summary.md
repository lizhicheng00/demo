...
所有 tunnel 查询、端口策略、JWT 签发、metering 都要落在当前 region 范围内。不能仅凭请求里传入的 gridName 或数据库里查到 tunnel 就信任，必须通过 grid-region 关系过滤。
...
- Gateway 内部接口：不使用 namespace，但必须通过 `gridName` 校验本地 grid 和 tunnel 归属。
...
Token 是 tunnel 访问凭证。当前是可复用 JWT，不是一次性 token，也不再提供独立 token API。
