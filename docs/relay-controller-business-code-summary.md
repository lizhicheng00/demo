...
Relay Controller 是 DevBridge / Relay Tunnel 系统中的控制面服务。它负责维护 tunnel 元数据、namespace 隔离、grid/region 归属校验、JWT token 签发、端口访问策略、流量计量上报和运行状态查询。
...
- tunnel 必须绑定到本 region 下的 grid。
- namespace 来自调用方传入的 `X-Namespace`，本服务不再维护 userId。
- tunnel id 是对 40-bit tunnel code 的 base32 表达。
- 删除采用软删，保留历史标识和计量关联。
- OpenAPI YAML 是接口契约源，Controller 继承 Maven 编译期生成的接口。
- Swagger UI 运行时依赖已删除，仅保留静态 `openapi.yaml` 和生成接口需要的 Swagger annotations。
...
- 接收 Gateway 或 grid 侧的 metering 上报。
- 查询 tunnel 当前 relay runtime status。
- 校验 namespace、region、grid、过期和删除状态。

### 2.2 Relay Gateway

Relay Gateway 是数据面组件。当前项目不实现 Gateway，但保留了 Gateway 所需的控制面接口：

- 端口策略查询：`GET /open-api-inner/v1/relay-controller/grids/{gridName}/tunnels/{tunnelId}/ports/{port}`
- 计量上报：`POST /open-api-inner/v1/relay-controller/grids/{gridName}/metering`
- token 中携带 `tunnelId`、`tunnelCode`、`namespace`、`gridName`，供 Gateway 识别连接授权。

Gateway port policy 接口保留 `gridName` 是有意设计。Gateway 调用时应以自己的 grid 作为作用域，Relay Controller 会确认 tunnel 属于这个 grid 后才返回端口策略，避免同 region 内跨 grid 查询策略。

### 2.3 Grid

Grid 是 tunnel 的部署/接入单元。每个 grid 归属一个 region。

当前本服务判断“是否本地 grid”的方式是查询 `grid` 表：

- `grid.grid = gridName`
- `grid.region = relay.region`

如果 grid 不属于当前 Relay Controller 实例的 region，会被视为不可访问。
...
所有 tunnel 查询、端口策略、token、metering、status 都要落在当前 region 范围内。不能仅凭请求里传入的 gridName 或数据库里查到 tunnel 就信任，必须通过 grid-region 关系过滤。

### 2.5 Namespace

Namespace 是租户或调用方隔离维度。当前服务读取 `X-Namespace` header 作为 namespace，不再使用 userId。

接口上有两种 namespace 语义：

- 控制台/租户侧接口：`X-Namespace` 必填。
- token 接口：`X-Namespace` 选填。传入时校验 tunnel 归属；不传时只校验 tunnel 存在、属于当前 region、未过期。
...
- `url`：外部访问 URL，目前格式为 `{tunnelId}-{gridName}-{relay.domain}`。
- `type`：`bridge` 或 `env`。
- `deleted`：软删标记。
...
Token 是 tunnel 访问凭证。当前是可复用 JWT，不是一次性 token。

关键规则：

- token 接口独立于 tunnel resource path：`POST /open-api-inner/v1/relay-controller/tokens`
- 请求体需要 `tunnelId`。
- `X-Namespace` 选填；传入则检查 namespace 归属。
...
Metering 是流量计量上报。调用方通常是 Gateway 或 grid 内部组件。
...
服务会校验 tunnel 属于当前 region、属于该 grid、未过期，并且请求里的 `tunnelCode` 和数据库 tunnelCode 一致。

### 2.10 Relay Status

Relay Status 是运行态状态查询。当前状态来自 Redis，而不是 MySQL。
...
如果 Redis 中没有记录，或 runtime status 的 gridName 与 tunnel 元数据不一致，则返回 `OFFLINE`。
...
- 删除 `springdoc-openapi-starter-webmvc-ui`，不再提供 Swagger UI。
...
静态资源不存在时，`NoResourceFoundException` 返回 HTTP `404`，并返回：
...
8. 构造 tunnel URL：`{tunnelId}-{gridName}-{relay.domain}`。
9. 插入 `tunnel` 表。
...
- 如果传入 `gridName`，先校验该 grid 属于当前 region。
...
返回字段不再包含冗余 `id`。原先 `id == tunnelId`，没有独立含义，已删除，只保留 `tunnelId`。
...
- body 中的 `tunnelId` 已 deprecated，Controller 会用 path 覆盖 request。
...
- 会 evict Redis 中 `jwt:token:{tunnelId}`，防止旧 token 有效期超过新 tunnel 生命周期。
...
- request body 可选，但实际业务需要 `tunnelId`，缺失会返回 `PARAM_INVALID`。
- tunnel 必须属于当前 region。
- tunnel 必须未删除、未过期。
- 如果传入 `X-Namespace`，额外校验 tunnel namespace。
...
1. 校验 path 中的 `gridName` 是当前 region 的本地 grid。
2. 按 `tunnelId + relay.region` 查 tunnel。
3. 校验 tunnel 未删除、未过期。
4. 校验 tunnel.gridName 等于 path gridName。
5. 校验 port 有配置。
6. 返回 `tunnelId`、`tunnelCode`、`gridName`、`port`、`allowAnonymous`。

为什么不删除这个接口：

- Gateway 不一定持有 namespace。
- Gateway 的天然作用域是 grid，而不是用户 namespace。
- path 中 gridName 可以防止同 region 内跨 grid 查询端口策略。
...
- `X-Namespace` 必填。
- tunnel 属于当前 region、namespace。
- tunnel 未删除、未过期。

状态来源：

- Redis key `relay:{tunnelId}`。

如果 Redis 没有状态记录，或状态记录里的 gridName 与 tunnel.gridName 不一致，返回：
...
- 数据库仍保留 `_id` 自增主键，但 API 不再返回 tunnel 的 `id` 字段。
...
value 为 JSON，反序列化为 `RelayStatus`。
...
- Gateway policy query 不要求 namespace，但要求 gridName 是本地 grid，且 tunnel 属于该 grid。
- 所有 port 操作都拒绝过期 tunnel。

### 10.3 TokenAppService

负责 token API 编排：

- 校验 request.tunnelId。
- 查询当前 region 下 active tunnel。
- 可选校验 namespace。
- 计算返回的 expiresIn。
- 调用 `JwtTokenService` 获取或生成 token。

注意：

- `expiresIn` 与实际 JWT/Redis TTL 对齐。
- `namespaceChecked` 日志字段表示是否执行了 namespace 归属校验。

### 10.4 JwtTokenServiceImpl
...
### 10.8 RelayStatusAppService

负责 tunnel runtime status 查询：

- 校验 namespace、region、过期状态。
- 从 Redis 读取 status。
- Redis 无记录或 grid mismatch 时返回 OFFLINE。

### 10.9 LocalGridService

负责本地 grid 判断：

- 按 `gridName + relay.region` 查询 grid。
- 查不到则抛 `GRID_NOT_FOUND`。
...
Controller 只实现生成接口，不再手写 mapping。这保证接口 path、method、参数来源主要由 YAML 控制。
...
- Redis relay status 是运行态缓存，失败或缺失视为 OFFLINE。
...
- region 由服务配置固定。
- grid 必须从数据库查出并匹配当前 region。
- token 可选 namespace 校验。
- Gateway policy 查询以 gridName 作为 scope，不使用 namespace。
...
- token namespace 校验、缺 tunnelId、跨 region 拒绝。
- metering 写入与 bandwidth 累加、跨 region grid 拒绝。
- relay status 缺 runtime record 返回 OFFLINE、grid mismatch 返回 OFFLINE。
- tunnel port create/list/update/delete/deleteAll/detail。
- gateway policy grid 校验和端口策略返回。
...
### 17.5 Gateway policy 保留 gridName

虽然 tunnel 能查到 gridName，但 Gateway 请求 path 中保留 gridName 是为了表达调用者 scope，并显式校验 tunnel 是否属于该 grid。

### 17.6 删除 tunnel 响应 id 字段

原先 tunnel response 中 `id == tunnelId`。这个字段没有独立业务含义，已删除，避免调用方误以为存在另一个 tunnel id。

## 18. 已删除或不再提供的能力

根据当前实现，以下能力不再属于 Relay Controller：

- node 同步接口。
- 一次性 token 接口。
- 证书获取接口。
- Swagger UI 页面。
- WebSocket/WebTransport/TCP/HTTP 数据转发。

这些能力如果后续需要，应重新评估是否属于 Relay Controller，还是应由 Relay Gateway 或其他服务承担。
...
6. `application/service/TokenAppService.java`
7. `domain/service/*`
8. `domain/repository/*`
9. `infrastructure/persistence/repository/*`
10. `src/main/resources/mapper/TunnelMapper.xml`
11. `infrastructure/security/*`
12. `infrastructure/redis/*`
13. `src/main/resources/db/migration/*`
14. `src/test/java/com/huawei/devbridge/relaycontroller/*Test.java`
...
2. 是否为 token 接口增加更明确的内部调用鉴权。
3. 是否对 `relay:{tunnelId}` runtime status 的 JSON schema 建立更严格契约。
4. 是否增加 tunnel 自动过期清理任务。
5. 是否增加 metering 聚合表，避免长期只存明细。
6. 是否支持多 JWT key 和 kid 轮换。
7. 是否需要为 `grid` 增加管理接口，或者由外部系统同步。
8. 是否为 Gateway policy API 增加调用来源校验。
9. 是否将 `expiration` 响应字段重命名为 `expiresAt`，减少“小时入参 / 秒响应”的理解成本。
10. 是否增加集成测试，覆盖真实 MySQL + Redis + Flyway 场景。
