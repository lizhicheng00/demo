...
- 第一版聚焦控制面业务闭环，范围包括 tunnel、port policy、gateway policy、metering、安全接入和基础治理。
- 数据面代理、复杂权限模型、证书管理、node 同步和 stats 统计不在第一版范围。
...
创建 tunnel 时，调用方指定所属 grid 和基础业务信息。服务端负责校验 grid 归属、namespace 配额和过期时间，并生成对外可使用的 tunnel 标识和访问域名。
...
当 tunnel 被删除或过期时间发生变化时，服务端清理 JWT 缓存。Redis 不可用时，接口仍可重新签发 JWT，不阻断 tunnel detail 查询。
...
| `gridName` | 绑定 grid |
| `expiration` | 过期时间 |
| `url` | tunnel 访问域名，格式为 `{tunnelId}-{gridName}-{relay.domain}` |
| `type` | tunnel 类型 |
...
| 字段 | 说明 |
| --- | --- |
| `jwt.tokenType` | token 类型 |
| `jwt.token` | 连接 JWT |
| `jwt.expiresIn` | JWT 剩余有效时间，单位秒 |
...
- tunnel 生命周期、tunnel port、gateway policy、metering 是第一版核心接口。
- tunnel detail 返回连接 JWT，调用方不需要额外的凭证获取流程。
